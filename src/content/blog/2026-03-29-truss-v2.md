...
_This post covers migrating our CSS-in-JS library, [Truss](https://github.com/homebound-team/truss/), from Emotion to a custom build-time solution, inspired by [StyleX](https://stylexjs.com/), implemented as a custom Vite plugin. If you want to jump straight to "what makes Truss great?", you can jump to [that section](#truss-v2-overview)._ 
...
Even though we didn't like StyleX-the-API, we did _really_ like StyleX-the-compiler, so we prototyped a version of Truss that, instead of "codegen DSL on top of Emotion", was a "codegen DSL on top of StyleX".
...
The key insight of Truss v2, that most differentiates us from StyleX, is that our Tachyons-inspired abbreviations, i.e. `df` for `display: flex` are _already unique_ and so can form the basis for our CSS class names, without any additional hashing or suffixing.
...
These are TypeScript files, but with a special `export const css` that is a map of "arbitrary selector" to "bag of styles", i.e. much like a regular CSS file:

```jsx
// In MyApp.css.ts, write arbitrary selectors as the keys,
// and the `selector { ...styles...}` will be appened to the
...
And finally the least performant/most flexible escape hatches is the `useRuntimeStyle` hook, which allows completely dynamic selectors:
...
It's slightly embarrassing to admit, but performance was not actually the primary motivation for our migration--it was derisking/reducing our stack's tech debt, by no longer relying on older/out-dated techniques.

But build-time CSS-in-JS is popular for a reason 😅, and we did see some nice performance improvements. 🚀
...
(Besides the LLM shout-out, huge thanks to the StyleX team for their innovative approach; as we've mentioned multiple times above, Truss v2 is basically a StyleX clone that better fits our goals and ergnomics.)
